    ...
    <div className='min-h-dvh w-full'>
      ...
      <div className="body bg-zinc-900 flex">
        ...
        <div className="container h-screen flex flex-col items-start bg-zinc-800 w-full overflow-y-auto">
          <div className="header w-full flex justify-between items-center m-3 p-3 gap-3">
            <h1 className='font-bold text-lg md:text-2xl'>Courses</h1>
            <button className='bg-blue-700 p-1 w-full sm:w-40 font-bold rounded-sm cursor-pointer hover:bg-blue-900'>
              Start Verification
            </button>
          </div>
          <div className="cards grid grid-cols-1 sm:grid-cols-2 gap-4 p-3 w-full">
            {courses.map((course) => (
              <CourseCard
                key={course.id}
                title={course.title}
